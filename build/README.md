...
| `--single-branch`              | All features share one `feat/<prefix>` branch; `/ship` + `/land-and-deploy` runs once after all features complete.                                                                     |
| `--ship-on-plan-complete`      | Keep per-feature branches but defer `/ship` + `/land-and-deploy` until every feature reaches `phases_done`. Then PRs open in feature order. Mutually exclusive with `--single-branch`. |
...
| `GSTACK_BUILD_FEATURE_REVIEW_MAX_ITER` | Feature-review loop cap (default 3, lowered from 5 because re-reviewing the same plan five times rarely converges; same-shape repeat detection halts earlier if the failure fingerprint matches across consecutive iterations).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `GSTACK_DISABLE_RECOVERY_REF`          | Set to `1` to skip the branch/commit recovery-ref safety net. By default, before any branch reset or force-delete that could strand reviewed-but-unlanded work (`cleanupLocalMergedBranch`'s `git branch -D`, `ensureOriginRetryBranch`'s switch onto a fresh-from-origin followup, and the release-daemon scratch `reset --hard`), `stampRecoveryRef` writes a `refs/gstack-recovery/<branch>-<sha>-<ts>` ref at the tip so the commit survives `git gc` and is recoverable by name (`git branch <new> <ref>`). Best-effort, never blocks the underlying op, and changes which branches get deleted by zero. List stamped refs with `git for-each-ref refs/gstack-recovery/`; they are not auto-pruned. Setting `=1` disables stamping entirely. |
...
| `orchestrator/recovery-ref.ts`              | Leaf helper: stamps `refs/gstack-recovery/` before branch-destroying ops. |
