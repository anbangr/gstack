...
prior commit.
   ...
   **Common defects to avoid.** The structural validator rejects plans that violate any of these five rules. Self-check against them before finishing:
   ...
   4. **Multi-arm split across features** — registry additions (e.g. "Add `X` to `child-registry.ts`") must land in the **same feature** as their orchestrator wiring (e.g. "Call `X` in `phase-runner.ts`"). Splitting them across features triggers a structural violation.
