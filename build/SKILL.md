...
5. **Synthesize living plan(s) and run manifest v2 (configured subagent)**: Delegate full plan synthesis to the configured `planSynthesizer` provider so the entire origin plan document is read off the main context. The subagent reads the source plan set and target repo list, writes one living plan per target repo/source plan, writes `$BUILD_TMP_DIR/build-run-manifest.json`, and returns only a compact summary.
   ...
   - **Co-located tests for `package main` shims**: When a `code` phase produces
     ...
     `experiments/x/cmd/foo/main.go`), the `<!-- testCmd: -->` MUST include the
     binary's directory in the path list. Go requires `package main` tests to live
     alongside `main.go`, so the test-writer will place `main_test.go` next to the
     binary. If the testCmd points only at the sibling `test/` tree, verify-red
     exercises the wrong directory, sees existing unit tests all-pass, and halts
     with `Gemini could not produce failing tests after N attempts
     (GSTACK_BUILD_RED_MAX_ITER)`. Concrete example: a phase that creates
     ...
     More generally: when a phase produces code whose tests cannot live in the
     repo's standard `test/` tree (Go `package main`, Python tests beside source,
     Rust integration tests under the crate's `tests/`), the testCmd MUST union
     the co-located directory and the standard one. List the co-located path
     first so a build failure there surfaces immediately.
