...
async function runPhase(args: {
  ...
  while (true) {
    ...
    if (action.type === "RUN_GEMINI_TEST_SPEC") {
      ...
      // inference. Reads the most recent commit's diff (the test-writer
      // hygiene gate requires a commit per iteration) and filters to
      // test-shaped paths. Best-effort: any failure leaves the field
      // empty and resolveTestCmdForPhase falls back to static resolution.
      if (!dryRun) {
        try {
