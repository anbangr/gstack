...
/**
 * Bug 3 (polis-mesh / mitosis-prototype / tidy-haven PREMATURE_COMPLETION):
 * the three-way gate every commit-transition MUST honor before flipping a
 * phase to `committed`. Centralizes the contract:
 *
 *   1. NOT timed out (stall watchdog SIGTERMed → soft failure).
 *   2. exitCode === 0 (process completed normally OR returned hygiene-fail
 *      shape — hygieneFailureResult returns exitCode: 1, so this gate
 *      naturally rejects hygiene failures too).
 *   3. NOT hygieneFailure (belt-and-suspenders: if a future refactor sets
 *      exitCode: 0 on a hygiene-rejected result, this still blocks the
 *      commit — the rule is "hygiene PASS required, not exit code 0").
 *
 * Existing phase-runner.ts sites use the equivalent `!timedOut &&
 * exitCode === 0` pattern (with the implicit hygieneFailure -> exitCode 1
 * coupling); this helper documents the contract and protects against
 * future drift where hygiene might decouple from exit code. New
 * commit-transition code MUST call shouldCommit(result) instead of
 * re-deriving the predicate.
 */
export function shouldCommit(result: SubAgentResult): boolean {
  if (result.timedOut) return false;
  if (result.exitCode !== 0) return false;
  if (result.hygieneFailure) return false;
  return true;
}
