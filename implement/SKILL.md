...
**ZERO-INTERRUPTION CONTRACT**: Once this loop starts, you MUST NOT stop between phases to ask for permission, confirmation, or guidance. This means you must NEVER say any of the following (or anything like them):
- "Shall I proceed to Phase X?"
- "Should I continue?"
- "Ready to move on?"
- "Let me know when you'd like me to start the next phase."
- "Phase X is complete. Should I begin Phase Y?"

If you catch yourself about to type any of those phrases, delete them and immediately execute the next phase instead. The only valid reason to stop and use `AskUserQuestion` is if a sub-agent fails catastrophically and you have exhausted all self-recovery attempts, or if you hit a hard safety constraint. **Finishing a phase is not a reason to pause.** Just keep going.
...
Do NOT stop to ask the user for permission between phases. See the zero-interruption contract at the top of this step — re-read it now if you are tempted to pause.
...
**Rules:**
- **Autonomous Continuity**: The zero-interruption contract above is absolute. Do NOT ask for confirmation between steps, phases, or loops. Do NOT announce a phase is done and wait. Do NOT ask "shall I proceed?". Narrate your state and immediately execute the next step.
