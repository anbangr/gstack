...
Run the deterministic loop script. It owns all phase iteration, sub-agent spawning, Codex review, checkbox patching, and context-saving — no manual step-through needed.

```bash
# Normal: execute all unchecked phases
bun run implement/loop.ts --plan=plans/<slug>-impl-plan-<date>.md

# Resume: same as normal — skip already-complete phases, continue from first [ ]
bun run implement/loop.ts --plan=plans/<slug>-impl-plan-<date>.md --mode=resume

# Reexamine: re-run every phase regardless of [x] status (full audit)
bun run implement/loop.ts --plan=plans/<slug>-impl-plan-<date>.md --mode=reexamine
```

Use the exact plan file path you wrote in Step 1. The script will:
1. For each unchecked `[ ] **Implementation` phase: spawn a `claude -p --dangerously-skip-permissions` sub-agent with the phase goal and commit instructions. The sub-agent uses its full tool suite and may delegate heavy coding to Gemini via `mcp__llm-bridge__ask_gemini` if available.
2. After each implementation: run `codex /gstack-review`.
3. Patch `[ ] → [x]` in the plan file after each sub-task.
4. Context-save at each phase boundary.
5. Exit when all phases are done, or exit non-zero with a clear resume command if a sub-agent fails.

**Do not restart from the beginning if a phase fails.** The error output will give you the exact `--mode=resume` command to continue from where it stopped.

**Mode selection:**
- Use `--mode=resume` after a mid-run failure or interrupted session.
- Use `--mode=reexamine` when the user says "reexamine", "audit", or "verify the implementation".
- Default (no flag) is equivalent to `--mode=normal`.
...
**Rules:**
- **Your job in Step 2 is to run the script, not to orchestrate phases yourself.** Do not re-implement the loop in prose. Pass the correct flags and let the script run.
- **Autonomous Skill Execution**: Skills MUST run as separate processes via the `Bash` tool. For shipping, use `claude --model sonnet -p /ship`. **NEVER invoke `review`, `qa`, or `ship` as native tools** — that dumps their source into your context and breaks the session.
- **Verbose State Reporting**: Tell the user what you are doing at each step (synthesizing, running the loop, shipping, etc.).
- **Strict adherence**: Stick to the plan. Do not expand scope unless strictly necessary to make the code compile.
- **Fail forward**: If the loop exits with an error, diagnose it and re-run with `--mode=resume`. Only escalate to the user if you are stuck after multiple attempts.
