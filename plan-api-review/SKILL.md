...
preamble-tier: 3
version: 1.0.0
description: Interactive API contract plan review. (gstack)
...
Tightens REST, gRPC, and lightweight
...
Proactively suggest when a plan changes public interfaces.
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-api-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
