...
describe("gstack-artifacts-init canonical URL storage (codex Finding #10)", () => {
  ...
  test("configures git origin with SSH form (derived from canonical HTTPS)", () => {
    ...
    const r = run(["--host", "github"]);
    expect(r.status).toBe(0);
    // Read the raw .git/config value, not `git remote get-url`, because the
    // developer's global git may have `url.<rewrite>.insteadOf=git@github.com:`
    // configured (a common pattern for forcing HTTPS auth on GitHub). That
    // rewrite hides the stored SSH form at read-time and breaks this test
    // assertion on machines that have the rewrite enabled, even though the
    // script correctly stored the SSH form. `git config --file` bypasses
    // insteadOf rewriting.
