...
export const SKILL_COVERAGE: Record<string, SkillCoverage> = {
  ...
  // Fork-only plan variants (anbangr fork).
  "plan-api-review": {
    gate: ["test/skill-coverage-floor.test.ts"],
    periodic: [],
    rationale:
      "Fork-only plan reviewer; floor coverage holds the structural compliance baseline.",
  },
  "plan-arch-review": {
    gate: ["test/skill-coverage-floor.test.ts"],
    periodic: [],
  ...
  // ─── Build orchestrator (fork-only) ────────────────────────
  build: {
    gate: ["test/skill-coverage-floor.test.ts"],
    periodic: [],
    rationale:
      "Fork-only autonomous /build skill; orchestrator has its own unit suite under build/orchestrator/__tests__/.",
  },
